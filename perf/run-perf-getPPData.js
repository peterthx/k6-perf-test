...
export default function () {
    group('- Get promptpay data successfully -', function () {
        const resCheckData = http.get(BASE_URL.URL_GET_PROMPTPAY);
        bodyLengthTrend.add(resCheckData.body.length);

        const isValidJson = check(resCheckData, {
        ...
        if (isValidJson) {
            const data = JSON.parse(resCheckData.body);
